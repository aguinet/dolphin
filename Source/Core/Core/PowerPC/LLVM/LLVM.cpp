...
struct LLVMJit::LLVMJitCtx
{
  ...
  struct FuncBuilder
  {
    FuncBuilder(const unsigned Id):
      Ctx_(std::make_unique<LLVMContext>()),
      M_(std::make_unique<Module>("jit", *Ctx_)),
      FRunTy(FunctionType::get(
        Type::getVoidTy(*Ctx_), {}, false)),
      IRB(BasicBlock::Create(*Ctx_, "entry",
            Function::Create(FRunTy, Function::ExternalLinkage, "F." + std::to_string(Id), M_.get())))
    {
  ...
  {
    return FuncBuilder{CurId_++};
  }

  uint8_t* Generate(FuncBuilder& FB) {
    StringRef Name = FB.LF()->getName();
    ExitOnErr(JIT_->addIRModule(ThreadSafeModule{std::move(FB.M_), std::move(FB.Ctx_)}));
...
void LLVMJit::LLVMJitCtx::FuncBuilder::CallFunc(ConditionalCallback const CB, u32 const V)
{
  ...
  Constant* F = ConstantExpr::getBitCast(
      ConstantInt::get(Type::getInt64Ty(Ctx), (uintptr_t)CB),
      FCondCBTy);
  Value* Ret = IRB.CreateCall(FCondCBTy, F,
